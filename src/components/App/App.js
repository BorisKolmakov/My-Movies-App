...
export default class App extends Component {
  ...
  getRatedFilms = () => {
    ...
    const { getRatedMovies } = this.mapiService
    this.getFilmsList(() => getRatedMovies(guestSessionId, currentPage))
  }
  ...
  getFilms = () => {
    ...
    const { getSearch, getPopular } = this.mapiService
    if (apiSearchFilm) {
  ...
  networkError = (err) => {
    ...
    if (!err) {
      ...
      Modal.success({
        content: 'Successfully connection',
      })
    }
  }
  render() {
    ...
    return (
      <React.Fragment>
        ...
        <Offline onChange={this.networkError} />
      </React.Fragment>
    )
  }
}
